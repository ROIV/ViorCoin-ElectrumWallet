...
"""
    ...
    def read_system_config(self):
        """Parse and store the system config settings in electrum-vior.conf into system_config[]."""
        name = '/etc/electrum-vior.conf'
        if os.path.exists(name):
            try:
                import ConfigParser
            except ImportError:
                print "cannot parse electrum-vior.conf. please install ConfigParser"
                return
