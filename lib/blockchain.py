...
try:
    from vior_scrypt import getPoWHash
except ImportError:
    print_msg("Warning: vior_scrypt not available, using fallback")
...
class Blockchain(threading.Thread):
    ...
    def __init__(self, config, network):
        ...
        self.local_height = 0
        self.running = False
        self.headers_url = 'http://electrum1.viorcoin.com/blockchain_headers'
    ...
    def get_target(self, index, chain=[]):
        ...
        # ViorCoin: go back the full period unless it's the first retarget
        if index == 1:
