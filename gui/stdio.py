...
_ = lambda x:x
#from i18n import _
from electrum_vior import mnemonic_encode, WalletStorage, Wallet
from electrum_vior.util import format_satoshis, set_verbosity
from electrum_vior.bitcoin import is_valid
from electrum_vior.network import filter_protocol
...
class ElectrumGui:
    ...
    def __init__(self, config, network):
        ...
        if not storage.file_exists:
            print "Wallet not found. try 'electrum-vior create'"
            exit()
    ...
    def do_send(self):
        if not is_valid(self.str_recipient):
            print(_('Invalid ViorCoin address'))
            return
        try:
    ...
    def network_dialog(self):
        print("use 'electrum-vior setconfig server/proxy' to change your network settings")
        return True


    def settings_dialog(self):
        print("use 'electrum-vior setconfig' to change your settings")
        return True
