...
from electrum_vior import mnemonic, pyqrnative, WalletStorage, Wallet

Gdk.threads_init()
APP_NAME = "Electrum-VIOR"
import platform
MONOSPACE_FONT = 'Lucida Console' if platform.system() == 'Windows' else 'monospace'

from electrum_vior.util import format_satoshis, parse_url
from electrum_vior.network import DEFAULT_SERVERS
from electrum_vior.bitcoin import MIN_RELAY_TX_FEE
...
class ElectrumWindow:
    ...
    def __init__(self, wallet, config, network):
        ...
        title = 'Electrum-VIOR ' + self.wallet.electrum_version + '  -  ' + self.config.path
    ...
    def do_send(self, w, data):
        ...
        if not is_valid(to_address):
            self.show_message( "invalid viorcoin address:\n"+to_address)
            return
